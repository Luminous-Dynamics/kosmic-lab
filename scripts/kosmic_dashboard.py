...
def load_track_d_data():
    ...
    try:
        ...
        sweep_files = list(Path('logs/track_d/parameter_sweep').glob('parameter_sweep_*.csv'))
        if sweep_files:
            ...
            ring_data = df[df['network_topology'] == 'ring'].sort_values('communication_cost').copy()
...
app.layout = html.Div([
    html.Div([
        html.H1("🌊 Kosmic Lab Real-Time Dashboard", 
                style={'textAlign': 'center', 'color': '#2c3e50'}),
        html.P("Live K-Index Monitoring Across All Experimental Tracks",
    ...
    # Track selector
    html.Div([
        html.Label("Select Track:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
        dcc.Dropdown(
            id='track-selector',
            options=[
                {'label': '🎮 Track B: SAC Controller (56 episodes)', 'value': 'track_b'},
                {'label': '⚡ Track C: Bioelectric Rescue (20 episodes)', 'value': 'track_c'},
                {'label': '🤝 Track D: Multi-Agent Coordination (Ring Topology)', 'value': 'track_d'},
    ...
    html.Div([
        html.Hr(),
        html.P("🌊 Kosmic Lab - Revolutionary AI-Accelerated Consciousness Research Platform",
...
def update_graphs(n, selected_track):
    ...
    k_dist.add_vline(x=1.5, line_dash="dash", line_color="red", 
    ...
    k_ts.add_hline(y=1.5, line_dash="dash", line_color="red",
                   annotation_text="Corridor", annotation_position="right")
    k_ts.update_layout(
    ...
    # Statistics Table
    stats_html = html.Div([
        html.P("Loading statistics...", style={'textAlign': 'center', 'color': '#95a5a6'})
    ])
    ...
    if selected_track == 'all':
        ...
        if data is not None and not data.empty and 'K' in data.columns:
            ...
            stats_html = html.Div([
                html.Table([
                    html.Tr([html.Td("Episodes:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{n_episodes}")]),
                    html.Tr([html.Td("Mean K:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{mean_k:.3f}")]),
                    html.Tr([html.Td("Std Dev:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{std_k:.3f}")]),
                    html.Tr([html.Td("Max K:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{max_k:.3f}")]),
                    html.Tr([html.Td("Min K:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{min_k:.3f}")]),
                    html.Tr([html.Td("% Corridor:", style={'fontWeight': 'bold'}), 
                            html.Td(f"{corridor_pct:.1f}%")])

...
def run_episode_with_fgsm(
    ...
    return {
        ...
        "fgsm_increased": (
            sum(1 for b, a in zip(base_losses, adv_losses) if a >= b) if base_losses else 0
        ),
...
def run_condition(
    ...
    print(
        f"Perturbation: {condition.perturbation_type}, strength={condition.perturbation_strength}, freq={condition.perturbation_frequency}"
    )
    print(f"{'='*80}\n")
    ...
    for episode in range(n_episodes):
        ...
        if (episode + 1) % config["experiment"]["save_interval"] == 0:
            k_robust_str = (
                f"K_p={episode_result['k_pearson']:.3f}, K_s={episode_result['k_spearman']:.3f}"
            )
            partial_str = (
                f"K_partial={episode_result['k_partial']:.3f} (Δ={episode_result['k_delta']:.3f})"
            )
            print(
                f"  Episode {episode+1}/{n_episodes}: K={episode_result['final_k']:.4f}, {k_robust_str}, {partial_str}, R={episode_result['episode_reward']:.2f}"
            )

            # FGSM sanity check
            if episode_result["fgsm_total"] > 0:
                fgsm_ok_pct = (
                    100.0 * episode_result["fgsm_increased"] / episode_result["fgsm_total"]
                )
                print(
                    f"    FGSM sanity: {episode_result['fgsm_increased']}/{episode_result['fgsm_total']} increased loss ({fgsm_ok_pct:.1f}%)"
                )
    ...
    print(
        f"  Robust: K_pearson={aggregated['mean_k_pearson']:.4f}, K_spearman={aggregated['mean_k_spearman']:.4f}"
    )
...
def generate_visualizations(all_results: List[Dict], output_dir: str):
    ...
    fig.suptitle(
        "Track F: Adversarial Robustness Analysis (Corrected FGSM)", fontsize=16, fontweight="bold"
    )
    ...
    bars = ax.bar(
        range(len(conditions)), ratios, alpha=0.7, color=sns.color_palette("mako", len(conditions))
    )
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(output_dir, "adversarial_robustness_summary.png"), dpi=300, bbox_inches="tight"
    )
...
def main():
    ...
    print(
        f"Total episodes: {len(config['adversarial_conditions']) * config['experiment']['n_episodes']}\n"
    )
    ...
    for i, r in enumerate(ranked, 1):
        ...
        print(
            f"  {i}. {r['condition']:25s} - {r['mean_final_k']:.4f} K ({ratio*100:.1f}% of baseline)"
        )

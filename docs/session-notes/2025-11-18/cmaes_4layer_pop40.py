#!/usr/bin/env python3
"""
🧠 4-Layer Pop=40: Push for All Seeds > 1.9

Pop=30/Gen=60 got 2/3 > 1.9. Can Pop=40 get 3/3?
"""
...
class CMAES:
    def __init__(self, dim, population_size=40, sigma=0.5):
        self.dim = dim
...
def run_single_seed(seed):
    np.random.seed(seed)
    cmaes = CMAES(dim=332, population_size=40, sigma=0.5)
    best = 0.0
    for _ in range(60):
        pop = cmaes.ask()
...
def main():
    print("╔════════════════════════════════════════════════════════════════╗")
    print("║  🧠 4-Layer Pop=40: Push for All Seeds > 1.9                  ║")
    print("╚════════════════════════════════════════════════════════════════╝\n")
    ...
    for seed in SEEDS:
        ...
        status = "🌟🌟🌟 K>1.9!" if best_k > 1.9 else ("🌟🌟 K>1.8!" if best_k > 1.8 else "🌟")
    ...
    print(f"\n📊 vs Pop=30:")
    print(f"  Pop=30: 1.8973 ± 0.0472 (2/3 > 1.9)")
    print(f"  Pop=40: {mean_k:.4f} ± {std_k:.4f} ({sum(1 for k in k_vals if k > 1.9)}/3 > 1.9)")

    if all(k > 1.9 for k in k_vals):
        print(f"\n  🌟🌟🌟✨ ALL SEEDS K > 1.9! ✨🌟🌟🌟")

    Path('logs/track_g_4layer_pop40').mkdir(parents=True, exist_ok=True)
    with open('logs/track_g_4layer_pop40/pop40_results.json', 'w') as f:
        json.dump({
            'results': results,
    ...
    print(f"\n📁 Saved to logs/track_g_4layer_pop40/")

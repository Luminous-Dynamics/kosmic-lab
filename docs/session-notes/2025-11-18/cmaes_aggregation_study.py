#!/usr/bin/env python3
"""
🧠 Aggregation Method Study: Mean vs Max vs Median vs Weighted
Tests different ways to combine ensemble outputs
"""
...
def aggregate_actions(actions, method='mean'):
    """Different aggregation methods for ensemble actions."""
    if method == 'mean':
        return np.mean(actions, axis=0)
    elif method == 'max':
        # Take action from network with highest norm (most confident)
        norms = [np.linalg.norm(a) for a in actions]
        return actions[np.argmax(norms)]
    elif method == 'median':
        return np.median(actions, axis=0)
    elif method == 'weighted':
        # Weight by action norm
        norms = np.array([np.linalg.norm(a) + 1e-8 for a in actions])
...
def evaluate(params, n_networks=3, obs_dim=8, act_dim=4, agg_method='mean'):
    ...
    for _ in range(4):
        ...
        for _ in range(80):
            ...
            action = aggregate_actions(acts, method=agg_method)
...
def run_aggregation_study():
    print("╔════════════════════════════════════════════════════════════════╗")
    print("║  🧠 Aggregation Method Study: Mean vs Max vs Median vs Weight ║")
    print("╚════════════════════════════════════════════════════════════════╝\n")

    METHODS = ['mean', 'max', 'median', 'weighted']
    N_NETWORKS = 3
    GENS = 30
    results = []

    print("Method   │  Best K  │ Status")
    print("─────────┼──────────┼──────────────")
    ...
    for method in METHODS:
        ...
        for _ in range(GENS):
            pop = cmaes.ask()
            fit = [evaluate(p, n_networks=N_NETWORKS, agg_method=method) for p in pop]
            best_k = cmaes.tell(pop, fit)
            if best_k > best:
                best = best_k

        results.append({'method': method, 'k': float(best)})
        status = "🌟 THRESHOLD!" if best > 1.5 else ("⭐ Close!" if best > 1.3 else "✓")
        print(f"{method:8s} │ {best:8.4f} │ {status}")

    print("\n" + "═" * 40)
    best_r = max(results, key=lambda x: x['k'])
    print(f"\n🎯 Best method: {best_r['method']}, K={best_r['k']:.4f}")

    # Save
    Path('logs/track_g_aggregation').mkdir(parents=True, exist_ok=True)
    with open('logs/track_g_aggregation/aggregation_study.json', 'w') as f:
        json.dump({'results': results, 'best': best_r, 'timestamp': datetime.now().isoformat()}, f, indent=2)

    print(f"\n📁 Saved to logs/track_g_aggregation/")
